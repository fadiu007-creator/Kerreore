...
import { ArrowLeft, CarFront, Check, Clock3, MapPin, ShieldCheck, Star } from "lucide-react";
...
export default function CarDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: revs } = await supabase.from("kerreore_reviews").select("rating,comment,created_at").eq("vehicle_id", id).eq("direction", "renter_to_owner").order("created_at", { ascending: false }).limit(5);
      setReviews(revs ?? []);
    })();
  }, [id]);
        ...
        <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
          <section>
            ...
            {reviews.length > 0 && (
              <div className="mt-6 rounded-[2rem] border border-black/8 bg-white p-6">
                <div className="flex items-center gap-2 font-black"><Star size={17} fill="currentColor"/> {avgRating?.toFixed(1)} \u00b7 {t(lang, "car_reviews_count", reviews.length)}</div>
                <div className="mt-4 space-y-3">{reviews.slice(0, 3).map((r, i) => (
                  <div key={i} className="rounded-2xl bg-[#f7f7f4] p-4 text-sm">
                    <div className="flex gap-0.5">{[1,2,3,4,5].map((n) => <Star key={n} size={13} fill={n <= r.rating ? "currentColor" : "none"}/>)}</div>
                    {r.comment && <p className="mt-1 text-black/60">{r.comment}</p>}
                  </div>
                ))}</div>
              </div>
            )}
          ...
          <aside className="lg:sticky lg:top-6 lg:self-start">
            <div className="rounded-[2rem] border border-black/8 bg-white p-6 shadow-xl">
              ...
              {avgRating && <div className="mt-2 flex items-center gap-1 text-sm font-bold"><Star size={14} fill="currentColor"/> {avgRating.toFixed(1)} \u00b7 {t(lang, "car_reviews_count", reviews.length)}</div>}
